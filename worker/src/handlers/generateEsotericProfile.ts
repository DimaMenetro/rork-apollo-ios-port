...
interface RequestBody {
  subject_id: string;
  subject_name?: string;
  esoteric_inputs: EsotericInputs;
  dsp_summary?: string;
}
...
function reduceNumerologyDigit(n: number): number {
...
export async function generateEsotericProfile(c: Context<{ Bindings: Env }>) {
  ...
  if (!esoteric_inputs) return c.json({ error: "esoteric_inputs required" }, 400);
...
CURRENT DATE: ${today} — Year: ${currentYear}, Month: ${currentMonth}. Use ONLY this date for all transit, progression, and personal year calculations. Do NOT use any other year.${personalYearNote}${timeframe ? `\nTimeframe: ${timeframe}` : ""}${focus ? `\nFocus: ${focus}` : ""}${dspSummary ? `\nDSP Summary: ${dspSummary}` : ""}`;
  ...
  const llm = (prompt: string, schema: Record<string, unknown>) =>
    invokeLLM(c.env, { model: "gemini_3_flash", prompt, response_json_schema: { type: "object", properties: schema } });
  ...
  try {
    ...
    const a = rA.response as Record<string, unknown>;
    const b = rB.response as Record<string, unknown>;
    const cc = rC.response as Record<string, unknown>;
    const d = rD.response as Record<string, unknown>;

    const sme = (d.sme_validation as Record<string, unknown> | undefined) ?? {};
    const esoteric_profile = {
      ...a, ...b, ...cc, ...d,
      input_fidelity: fidelityState,
      execution_status: (sme.execution_status as string) || "COMPLIANT",
      date_executed: today,
      include_in_dsp: false,
    };

    return c.json({ success: true, esoteric_profile });
  } catch (e: any) {
    return c.json({ error: e?.message ?? "esoteric generation failed" }, 500);
  }
}
